...
% ✅ ADAPTIVE TOP-K VALUES (based on available results)
num_bahn_results = length(bahn_results);
num_segment_results = 0;
if ~isempty(all_segment_results{1})
    num_segment_results = length(all_segment_results{1}.results);
end

% Define all possible K values
all_k_values = [5, 10, 25, 50];

% ✅ Filter K values based on available results
top_k_test_values = [];
for k_val = all_k_values
    if k_val <= num_bahn_results
        top_k_test_values = [top_k_test_values, k_val];
    end
end

fprintf('Available results: %d bahnen, %d segments\n', num_bahn_results, num_segment_results);
fprintf('Testing Top-K values: [%s]\n\n', strjoin(string(top_k_test_values), ', '));
...
fprintf('╔════════════════════════════════════════════════════════════════╗\n');
fprintf('║  SECTION 5: PERFORMANCE PROGNOSIS RESULTS (k-NN Regression)   ║\n');
fprintf('╚════════════════════════════════════════════════════════════════╝\n\n');

% ========================================================================
% COLLECT METRICS FOR OVERVIEW
% ========================================================================

overview_data = struct();
overview_data.k_values = [];
overview_data.bahn_s1_simple = [];
overview_data.bahn_s1_weighted = [];
overview_data.bahn_s2_simple = [];
overview_data.bahn_s2_weighted = [];
overview_data.seg_s1_simple = [];
overview_data.seg_s1_weighted = [];
overview_data.seg_s2_simple = [];
overview_data.seg_s2_weighted = [];
...
fprintf('═══════════════════════════════════════════════════════════════\n');
fprintf('ACTUAL PERFORMANCE (SIDTW Average Distance)\n');
fprintf('═══════════════════════════════════════════════════════════════\n');

% Bahn
if ~isempty(bahn_prog_stage1) && isfield(bahn_prog_stage1, 'actual')
    fprintf('  Bahn %-20s: %.4f mm\n', query_id, bahn_prog_stage1.actual.average_distance);
end

% Segments
for seg_idx = 1:num_query_segments
    if ~isempty(segment_prog_stage1{seg_idx}) && isfield(segment_prog_stage1{seg_idx}, 'actual')
        seg_id = query_data.segment_ids{seg_idx};
        fprintf('  Segment %-20s: %.4f mm\n', seg_id, segment_prog_stage1{seg_idx}.actual.average_distance);
    end
end
    ...
    for k_idx = 1:length(k_fields)
        ...
        if isfield(bahn_prog_stage2.predictions, k_field)
            ...
            fprintf('  Impr (Weighted vs Simple):    %.4f mm (%.1f%%) %s\n', ...
                s2_weighted_impr, s2_weighted_impr_pct, ternary(s2_weighted_impr > 0, '✓', '✗'));
            ...
            if ~isempty(segment_prog_stage1{seg_idx}) && ...
               ...
               isfield(segment_prog_stage2{seg_idx}.predictions, k_field)
                ...
                fprintf('  → Wtd vs Simple:      %.4f mm (%.1f%%) %s\n', ...
                    s2_seg_weighted_impr, s2_seg_weighted_impr_pct, ternary(s2_seg_weighted_impr > 0, '✓', '✗'));
        ...
        if ~isempty(seg_s1_simple_errors)
            ...
            overview_data.seg_s1_weighted = [overview_data.seg_s1_weighted; mean(seg_s1_weighted_errors)];
            overview_data.seg_s2_simple = [overview_data.seg_s2_simple; mean(seg_s2_simple_errors)];
...
% ================================================================
% KEY FINDINGS (Option C: Range + Context + Best K) - FIXED VERSION
% ================================================================
...
% Calculate ranges for weighted improvement
s1_wtd_impr_range = (overview_data.bahn_s1_simple - overview_data.bahn_s1_weighted) ./ overview_data.bahn_s1_simple * 100;
s2_wtd_impr_range = (overview_data.bahn_s2_simple - overview_data.bahn_s2_weighted) ./ overview_data.bahn_s2_simple * 100;

fprintf('1. Distance Weighting (Dudani 1976):\n');
fprintf('   Stage 1: %.1f%% to %.1f%% improvement (avg: %.1f%%)\n', ...
    min(s1_wtd_impr_range), max(s1_wtd_impr_range), mean(s1_wtd_impr_range));
[~, best_s1_idx] = max(s1_wtd_impr_range);
fprintf('   → Best at K=%d: %.1f%% %s\n', ...
    overview_data.k_values(best_s1_idx), s1_wtd_impr_range(best_s1_idx), ...
    ternary(s1_wtd_impr_range(best_s1_idx) > 0, '✓', ''));

fprintf('   Stage 2: %.1f%% to %.1f%% improvement (avg: %.1f%%)\n', ...
    min(s2_wtd_impr_range), max(s2_wtd_impr_range), mean(s2_wtd_impr_range));
[~, best_s2_idx] = max(s2_wtd_impr_range);
fprintf('   → Best at K=%d: %.1f%% %s\n', ...
    overview_data.k_values(best_s2_idx), s2_wtd_impr_range(best_s2_idx), ...
    ternary(s2_wtd_impr_range(best_s2_idx) > 0, '✓', ''));

fprintf('   Conclusion: Weighting consistently effective across all K\n');
fprintf('\n');

% DTW Reranking
s1_to_s2_simple_range = (overview_data.bahn_s1_simple - overview_data.bahn_s2_simple) ./ overview_data.bahn_s1_simple * 100;
s1_to_s2_wtd_range = (overview_data.bahn_s1_weighted - overview_data.bahn_s2_weighted) ./ overview_data.bahn_s1_weighted * 100;

fprintf('2. DTW Reranking (Stage 2):\n');
fprintf('   With Simple:   %.1f%% to %.1f%% (avg: %.1f%%)\n', ...
    min(s1_to_s2_simple_range), max(s1_to_s2_simple_range), mean(s1_to_s2_simple_range));
fprintf('   With Weighted: %.1f%% to %.1f%% (avg: %.1f%%)\n', ...
    min(s1_to_s2_wtd_range), max(s1_to_s2_wtd_range), mean(s1_to_s2_wtd_range));

if mean(s1_to_s2_wtd_range) > 0
    [~, best_dtw_idx] = max(s1_to_s2_wtd_range);
    fprintf('   → Best at K=%d: +%.1f%% ✓\n', ...
        overview_data.k_values(best_dtw_idx), s1_to_s2_wtd_range(best_dtw_idx));
    fprintf('   Conclusion: DTW helpful at large K\n');
else
    [~, worst_dtw_idx] = min(s1_to_s2_wtd_range);
    fprintf('   → Worst at K=%d: %.1f%% ✗\n', ...
        overview_data.k_values(worst_dtw_idx), s1_to_s2_wtd_range(worst_dtw_idx));
    fprintf('   Conclusion: Multimodal embeddings already optimal;\n');
    fprintf('               DTW reranking harmful at small K\n');
end
fprintf('\n');

% Best method overall
% ✅ FIX: Richtige Matrix-Struktur
% Rows = K values, Columns = Methods
all_methods = [overview_data.bahn_s1_simple, ...
               overview_data.bahn_s1_weighted, ...
               overview_data.bahn_s2_simple, ...
               overview_data.bahn_s2_weighted];
method_names = {'S1 Simple', 'S1 Weighted', 'S2 Simple', 'S2 Weighted'};

[min_err, min_idx] = min(all_methods(:));
[k_idx, method_idx] = ind2sub(size(all_methods), min_idx);

[max_err, ~] = max(all_methods(:));

fprintf('3. Overall Best Method:\n');
fprintf('   Winner: %s @ K=%d\n', method_names{method_idx}, overview_data.k_values(k_idx));
fprintf('   MAE: %.4f mm (%.1f%% better than worst)\n', ...
    min_err, (1 - min_err/max_err) * 100);
fprintf('\n');
...
fprintf('SEGMENT-LEVEL (avg over %d segments):\n', num_query_segments);
fprintf('─────────────────────────────────────────────────────────────\n');

% Calculate ranges
seg_s1_wtd_range = (overview_data.seg_s1_simple - overview_data.seg_s1_weighted) ./ overview_data.seg_s1_simple * 100;
seg_s2_wtd_range = (overview_data.seg_s2_simple - overview_data.seg_s2_weighted) ./ overview_data.seg_s2_simple * 100;

fprintf('1. Distance Weighting:\n');
fprintf('   Stage 1: %.1f%% to %.1f%% improvement (avg: %.1f%%)\n', ...
    min(seg_s1_wtd_range), max(seg_s1_wtd_range), mean(seg_s1_wtd_range));
fprintf('   Stage 2: %.1f%% to %.1f%% improvement (avg: %.1f%%)\n', ...
    min(seg_s2_wtd_range), max(seg_s2_wtd_range), mean(seg_s2_wtd_range));
fprintf('\n');

% DTW
seg_s1_to_s2_range = (overview_data.seg_s1_weighted - overview_data.seg_s2_weighted) ./ overview_data.seg_s1_weighted * 100;

fprintf('2. DTW Reranking:\n');
fprintf('   %.1f%% to %.1f%% (avg: %.1f%%)\n', ...
    min(seg_s1_to_s2_range), max(seg_s1_to_s2_range), mean(seg_s1_to_s2_range));

if mean(seg_s1_to_s2_range) > 0
    fprintf('   Conclusion: DTW helpful for segments\n');
else
    fprintf('   Conclusion: Multimodal embeddings already optimal\n');
end
fprintf('\n');

% Best method - ✅ FIX: Same as above
all_seg_methods = [overview_data.seg_s1_simple, ...
                   overview_data.seg_s1_weighted, ...
                   overview_data.seg_s2_simple, ...
                   overview_data.seg_s2_weighted];

[min_seg_err, min_seg_idx] = min(all_seg_methods(:));
[k_seg_idx, method_seg_idx] = ind2sub(size(all_seg_methods), min_seg_idx);

fprintf('3. Overall Best Method:\n');
fprintf('   Winner: %s @ K=%d\n', method_names{method_seg_idx}, overview_data.k_values(k_seg_idx));
fprintf('   MAE: %.4f mm\n', min_seg_err);
fprintf('\n');
