...
for db_idx = 1:length(database_sizes)
    ...
    for mode_idx = 1:length(unique_dtw_modes)
        ...
        estimated_calls = round(db_size * dtw_config.lb_kim_keep_ratio);
        estimated_calls = min(estimated_calls, dtw_config.lb_keogh_candidates);
        
        calls_key = [baseline_key '_calls_per_query'];
        dtw_baselines.(calls_key) = estimated_calls;
        ...
        for k_idx = 1:num_k_values
            ...
            for db_idx = 1:length(database_sizes)
                ...
                for q_idx = 1:num_queries
                    ...
                    dtw_baseline_calls_key = sprintf('%s_db_%d_calls_per_query', dtw_mode_curr, db_size);
                    if isfield(dtw_baselines, dtw_baseline_calls_key)
                        baseline_dtw_calls = dtw_baselines.(dtw_baseline_calls_key);
                    else
                        baseline_dtw_calls = db_size;  % Fallback
                    end
                    ...
                    used_lb_kim_traj, used_lb_keogh_traj, baseline_dtw_calls);
                    ...
                    {}, {}, ...
                    used_lb_kim_seg_agg, used_lb_keogh_seg_agg, baseline_dtw_calls);
...
% Pre-allocate arrays
timestamp_arr = cell(num_results, 1);
...
embedding_only_rk = zeros(num_results, 1);
...
twostage_rk = zeros(num_results, 1);
...
% Baseline comparison
baseline_dtw_r1 = zeros(num_results, 1);
baseline_dtw_r5 = zeros(num_results, 1);
baseline_dtw_r10 = zeros(num_results, 1);
baseline_dtw_r50 = zeros(num_results, 1);
baseline_dtw_rk = zeros(num_results, 1);
...
for i = 1:length(all_results)
    ...
    % Configuration
    timestamp_arr{result_idx} = timestamp;
    ...
    embedding_only_rk(result_idx) = r.embedding_only_rk;
    ...
    twostage_rk(result_idx) = r.recall_at_k;
    ...
    % Baseline
    baseline_dtw_r1(result_idx) = r.baseline_r1;
    baseline_dtw_r5(result_idx) = r.baseline_r5;
    baseline_dtw_r10(result_idx) = r.baseline_r10;
    baseline_dtw_r50(result_idx) = r.baseline_r50;
    baseline_dtw_rk(result_idx) = r.baseline_rk;
